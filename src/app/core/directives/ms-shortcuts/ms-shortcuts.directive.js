import template from './ms-shortcuts.html';
import angular from 'angular';

function MsShortcutsController(
	$scope,
	$cookies,
	$document,
	$timeout,
			...
			results = results.filter(function(item) {
				if (
					angular
						...
						.search(angular.lowercase(vm.query)) > -1
				) {
					return true;
				}
			});
		...
		const shortcuts = [
			{
				...
				children: [],
				_id: 'patients',
				_path: 'app.admin_patients',
				uisref: 'app.admin_patients',
				hasShortcut: true,
			},
		];
	...
	function ensureSelectedResultIsVisible() {
		var resultsEl = $document
				.find('#ms-shortcut-add-menu')
				.find('.results'),
			selectedItemEl = angular.element(
				resultsEl.find('.result')[vm.selectedResultIndex]
			);
...
export { MsShortcutsController, msShortcutsDirective };
